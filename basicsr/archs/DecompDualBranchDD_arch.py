...
class DecompDualBranch2DD(nn.Module):
    """
    DecompDualBranch2DD:
    Takes a 6-channel input: first 3 are image, next 3 are conditioning.
    Decompose both image and conditioning into Q1/Q2 quaternions (each Nx4xHxW).
    
    Then:
      Q1 = concat(Q1_img, Q1_cond) -> Nx8xHxW
      Q2 = concat(Q2_img, Q2_cond) -> Nx8xHxW

    These are processed by dual-branch encoders/decoders. 
    At the end, Q1_out and Q2_out are reduced back to Nx4xHxW and combined via Hamilton product to produce a 3-channel final output.

    Output:
      [x (6 channels), final_out (3 channels)]
    """
    ...
    def __init__(
        ...
        super(DecompDualBranch2DD, self).__init__()
        self.stage = stage
        ...
        # Now Q1 and Q2 each have 8 channels (4 from image + 4 from conditioning)
        in_channels_branch = 8
        out_channels_branch = 4  # Final quaternion output is always 4 channels
    ...
    def forward(self, x, mask=None):
        ...
        with torch.no_grad():
            ...
            Q1_cond, Q2_cond = self.decomp(cond)# Nx4xHxW

        # Concatenate Q1_img and Q1_cond along channel dimension: Nx8xHxW
        Q1 = torch.cat([Q1_img, Q1_cond], dim=1)
        # Concatenate Q2_img and Q2_cond along channel dimension: Nx8xHxW
        Q2 = torch.cat([Q2_img, Q2_cond], dim=1)
