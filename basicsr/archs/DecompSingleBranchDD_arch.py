...
class DecompSingleBranchDD(nn.Module):
    """
    A variant of DecompSingleBranch that also decomposes the conditioning signal.
    Inputs: Nx6xHxW (3-ch image + 3-ch conditioning)
    Steps:
      1) Decompose both image and conditioning using the same decomposition model.
      2) For image: Q1_img, Q2_img (each Nx4xHxW)
         For cond: Q1_cond, Q2_cond (each Nx4xHxW)
      3) Concatenate Q1_img, Q2_img, Q1_cond, Q2_cond => Nx16xHxW
      4) Pass through UNet-like structure (modified for 16 input channels).
      5) Output Nx8xHxW, split into Q1_out and Q2_out and apply Hamilton product.
      6) Produce final Nx3xHxW image.
    """
    ...
    def __init__(
        ...
        super(DecompSingleBranchDD, self).__init__()
        self.stage = stage
        ...
        # Now we have Q1_img,Q2_img (8 channels) + Q1_cond,Q2_cond (8 channels) = 16 channels total input
        in_channels_unet = 16
        out_channels_unet = 8
    ...
    def forward(self, x, mask=None):
        ...
        # Decompose image
        with torch.no_grad():
            Q1_img, Q2_img = self.decomp(img)   # Nx4xHxW each

        # Decompose conditioning
        with torch.no_grad():
            Q1_cond, Q2_cond = self.decomp(cond) # Nx4xHxW each

        # Concatenate all: Nx16xHxW
        fea = torch.cat([Q1_img, Q2_img, Q1_cond, Q2_cond], dim=1)
        ...
        # Perform Hamilton product to get final image
