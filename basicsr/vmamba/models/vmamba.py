...
##### Gated-Dconv MLP (gdMlp)
class gdMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., channels_first=False):
        super(gdMlp, self).__init__()
...
class FFNv02(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., channels_first=False):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(in_features, hidden_features, 1, 1, bias=False),
            GELU(),
            nn.Conv2d(hidden_features, hidden_features, 3, 1, 1, bias=False, groups=hidden_features),
            GELU(),
            nn.Conv2d(hidden_features, out_features, 1, 1, bias=False),
        )
    def forward(self, x):
        """
        x: [b,h,w,c]
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self._forward = self._forwardv01 if not grid_size else self._forwardv02
    ...
    def _forwardv01(self, input: torch.Tensor):
        """
        peforming full resolution SS2D
        """
        x = input
        if self.ssm_branch:
            if self.post_norm:
    ...
    def _forwardv02(self, input: torch.Tensor):
        """
        peforming windowed SS2D
        """
        x = input
        if self.ssm_branch:
            x0 = x
            ...
            pad_l = pad_t = 0
            pad_r = (self.grid_size - W % self.grid_size) % self.grid_size
            pad_b = (self.grid_size - H % self.grid_size) % self.grid_size
            x = F.pad(x, (pad_l, pad_r, pad_t, pad_b))
            _, _, Hp, Wp = x.shape

            if self.post_norm:
                windows_x = window_partition(x, self.grid_size)
                windows_x = self.op(windows_x)
                x = window_reverse(windows_x, self.grid_size, Hp, Wp)
                if pad_r > 0 or pad_b > 0:
                    x = x[:, :, :H, :W].contiguous()
                x = x0 + self.drop_path(self.norm(x))
            else:
                x = self.norm(x)
                windows_x = window_partition(x, self.grid_size)
                windows_x = self.op(windows_x)
